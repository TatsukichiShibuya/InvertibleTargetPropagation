...
class mytp_net(net):
    ...
    def train(self, train_loader, valid_loader, epochs, stepsize, lr_ratio, lrb, scaling,
        ...
        for e in range(epochs):
            ...
            target_dist_u = [[] for d in range(self.depth - self.direct_depth)]
            target_dist_d = [[] for d in range(self.depth - self.direct_depth)]
            ...
            for x, y in train_loader:
                ...
                for d in range(self.depth - self.direct_depth):
                    ...
                    target_dist_u[d].append((torch.norm(v3, dim=1)[0])
                    target_dist_d[d].append((torch.norm(v2, dim=1)[0]))
            ...
            with torch.no_grad():
                ...
                if log:
                    ...
                    for d in range(self.depth - self.direct_depth):
                        ...
                        print(
                            f"\ttarget err angle u {d}: {torch.mean(torch.tensor(target_angle_u[d]))}")
                        print(
                            f"\ttarget err angle d {d}: {torch.mean(torch.tensor(target_angle_d[d]))}")
